function [sinusoidal,partial,amplitude,frequency,phase] = sm_sinusoidal_resynthesis_OLA(amp,freq,ph,framelen,hop,fs,nsample,center_frame,...
...
for iframe = 1:nframe
    ...
    [amplitude_frame(:,iframe,:),frequency_frame(:,iframe,:),phase_frame(:,iframe,:),synthwin] = sm_stationary_synthesis(amp(:,iframe),freq(:,iframe),ph(:,iframe),framelen,fs,center_frame(iframe),winflag);
    
    % OLA resynthesis
    [time_frame(:,iframe),partial_frame(:,iframe,:)] = sm_OLA_resynthesis(squeeze(amplitude_frame(:,iframe,:)),squeeze(phase_frame(:,iframe,:)),synthwin);
    
end

% OLA scaling factor
sc = sm_colasum(winflag)*(framelen/2)/hop;

% Overlap-add time_frame
sinusoidal = sm_ola(time_frame,framelen,hop,nsample,center_frame,nframe,winflag,cfwflag)/sc;

% Overlap-add partials
partial = sm_ola(partial_frame,framelen,hop,nsample,center_frame,nframe,winflag,cfwflag)/sc;

% Overlap-add amplitude
amplitude = sm_ola(amplitude_frame,framelen,hop,nsample,center_frame,nframe,winflag,cfwflag)/sc;

% Overlap-add frequency
frequency = sm_ola(amplitude_frame,framelen,hop,nsample,center_frame,nframe,winflag,cfwflag)/sc;

% Overlap-add phase
phase = sm_ola(amplitude_frame,framelen,hop,nsample,center_frame,nframe,winflag,cfwflag)/sc;

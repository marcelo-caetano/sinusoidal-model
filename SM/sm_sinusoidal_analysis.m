function [amp,freq,ph,nsample,dc,center_frame,npartial,nframe] = sm_sinusoidal_analysis(wav,framelen,hop,nfft,fs,maxnpeak,relthres,absthres,delta,...
...
% Short-Time Fourier Transform
[fft_frame,nsample,dc,center_frame] = sm_stft(wav,framelen,hop,nfft,winflag,cfwflag,normflag,zphflag);

% Number of frames
nframe = sm_numframe(nsample,framelen,hop,cfwflag);

% Scale the magnitude spectrum (Linear, Log, Power)
[mag_spec,p] = sm_scale_magspec(fft_frame,framelen,nfft,winflag,magflag);

% Number of positive frequency bins
nbin = sm_nyq(nfft);

% Unwrap the phase spectrum
ph_spec = sm_phase_unwrap(fft_frame,nfft);
...
% Peak picking
[amp_peak,freq_peak,ph_peak] = sm_peak_picking(mag_spec,ph_spec,nfft,fs);

% Magnitude interpolation (quadratic)
[frequency,amplitude] = sm_mag_interp(freq_peak,amp_peak);

% Unscale magnitude spectrum
amplitude = sm_unscale_magspec(amplitude,p,magflag);

% Phase interpolation (linear)
phase = sm_phase_interp(freq_peak,ph_peak,frequency);

% Return only the MAXNPEAK highest amplitude peaks in MAG SPEC
[amplitude,frequency,phase] = sm_maxnumpeak(amplitude,frequency,phase,maxnpeak,nbin,nframe);

% Absolute threshold (corrected for full spectral energy)
[amplitude,frequency,phase] = sm_absdb(amplitude,frequency,phase,absthres);

% Relative threshold
[amplitude,frequency,phase] = sm_reldb(amplitude,frequency,phase,relthres);

% Handle partial tracking
if ~isempty(ptrackflag)
    
    % Partial tracking
    [amp,freq,ph,npartial] = sm_partial_tracking(amplitude,frequency,phase,delta,hop,fs,nframe,ptrackflag);

function [time_frame,nsample,dc,centerframe] = sm_sof(wav,framelen,hop,winflag,cfwflag,normflag)
...
% Execute SOF
[time_frame] = sofexe(wav,nsample,analysis_window,framelen,centerframe,nframe,nchannel);
...
analysis_window = sm_mkcolawin(framelen,winflag);
...
centerframe = sm_f2s(1:nframe,cfw(framelen,cfwflag),hop);

end

% FUNCTION THAT EXECUTES SOF
function [windowed_frames,nsample,centerframe] = sofexe(wav,nsample,analysis_window,framelen,centerframe,nframe,nchannel)
...
for iframe = 1:nframe
    ...
    % Make each frame
    time_frame(:,iframe,:) = mkframe(wav,centerframe(iframe),framelen,nsample,nchannel);

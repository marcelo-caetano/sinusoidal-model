function [p,t,s] = sm_swipep_mod(x,fs,plim,dt,dlog2p,dERBs,woverlap,sTHR)
...
for i = 1 : length(ws)
    ...
    % Octave complains about SPECGRAM here
    ...
    if size(Si,2) > 1
        ...
        % WARNING: Replaced ti for reproducibility
        % Si = interp1( ti, Si', t, 'linear', NaN )';
        Si = interp1([0;ti(1:end-1)'], Si', t, 'linear', NaN )';

function linmagspec = fft2lin_mag_spec(fft_frame,nfft,posfreqflag,nrgflag)
...
%
%   LMS = FFT2LIN_MAG_SPEC(FFT,NFFT,POSFREQFLAG) uses the logical flag
%   POSFREQFLAG to specify the frequency range of LMS. POSFREQFLAG = FALSE
%   is the default to output the full frequency range of the linear
%   magnitude spectrum. POSFREQFLAG = TRUE forces FFT2LOG_MAG_SPEC to
...
%
%   LMS = FFT2LIN_MAG_SPEC(FFT,NFFT,POSFREQFLAG,NRGFLAG) uses the logical
...
%   does not. The default is NRGFLAG = FALSE. POSFREQFLAG must be TRUE when

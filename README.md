...
- Refactored spectrum scaling functions and added *power scaling quadratic interpolation* to the sinusoidal parameter estimation methods

### New functions/features

- Function `freq_matching.m` was completely refactored into several local functions called as subroutines

- Partial tracking step has a dedicated function `partial_tracking.m`, which calls the functions `peak2peak_freq_matching.m` and `freq_matching.m`

- Introduced power scaling sinusoidal parameter estimation in functions `xqifft.m` and `interp_pow_scaling.m`
...
- Introduced subroutine `parameter_estimation.m`

- Updated function `peak_picking.m` inside *parameter estimation* to handle regular and symmetrical spectral peaks

- Standardized all flags inside the code base

### Deprecated functions/features

- Deprecated function `maxnumpeak.m`

### Backwards compatibility

### Bug fixes

- Fixed bug in function `maxnumpeak.m` and replaced it by core function `tools.sin.maxnumpeak.m` and high-level function `max_num_peak.m`
